			...
			if(int(self.request.get(type))):
				...
				for x in range (0, int((self.request.get(type)))):
					if(seatNumbersLength>seatNumbersIndex):
						...
						TicketReservation(parent=showrecord, seat_number=seatNumbers[seatNumbersIndex],ticket_type=str.upper(type.split('_')[0]+"_"+type.split('_')[1]),show_record_id=show_id,late_sale=latesale).put()
