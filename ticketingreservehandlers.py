...
class MoneyHandler(BaseHandler):
    ...
    @user_required
    def get(self):
        template_values = self.session['reprint_data']
        return self.render_template('sample-reserve.html', template_values)
...
class TicketingReserveHandler(BaseHandler):
    ...
    def get(self):
        ...
        if (showrecord):
            ...
            if (ticketreserve):
                ...
                setReservedList = list(set(','.join(reservedSeatList)))
            if (total_ticket_sale != len(reservedSeatList)):
                logging.critical('ticket sale mismatch***' + str(total_ticket_sale) + 'reservations' + str(len(reservedSeatList)))
            if (len(setReservedList) != len(reservedSeatList)):
                logging.critical('reservation duplicated in DB***' + seatListOutString)
        ...
        no_late_types = ['box_full', 'balcony_full', 'balcony_half', 'balcony_service', 'balcony_complimentary',
                         'odc_full', 'odc_half', 'odc_service',
                         'odc_complimentary', 'firstclass_full', 'firstclass_half', 'firstclass_service',
    ...
    def post(self):
        ...
        ticketprint = {}
        #seatNumbers=self.request.get('seatListString').split(',')
        seatNumbers = self.request.get('seatListString').split(',') # deduplication n splitting
        if (len(seatNumbers) != len(list(set(self.request.get('seatListString').split(','))))):
            logging.critical(
                'reservation duplicated in seatplan data***' + self.request.get('seatListString').split(','))
        seatNumbers = list(set(self.request.get('seatListString').split(',')))
        seatNumbersLength = len(seatNumbers)
        seatNumbersIndex = 0
        for type in types:
            type_inc = 0
            if (latesale):
            ...
            if not (ticketsale):
                ...
                #add user and date time of the sale to ticketsale
            ...
            if (int(self.request.get(type))):
                ...
                for x in range(0, int((self.request.get(type)))):
                    if (seatNumbersLength > seatNumbersIndex):
